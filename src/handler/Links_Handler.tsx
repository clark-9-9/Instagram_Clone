...
const handle_search_box = (
    ...
    if(screenWidth < 768) {
        ...
        setSearchBoxMargin((prevMargin) => prevMargin === "-100%" ? "0" : "-100%");
        setNotificationBoxMargin("-100%");
    } 

    if(screenWidth >= 768) {
        setSearchBoxMargin((prevMargin) => prevMargin === "-25rem" ? "79px" : "-25rem");
...
const handle_notification_box = (
    ...
    if(screenWidth < 768) {
        setNotificationBoxMargin((prevMargin) => prevMargin === "-100%" ? "0" : "-100%");
        setSearchBoxMargin("-100%");
    }

    if(screenWidth >= 768 ) {
        setNotificationBoxMargin((prevMargin) => prevMargin === "-25rem" ? "79px" : "-25rem");
